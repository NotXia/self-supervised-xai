...
class _FusionModel(L.LightningModule):
    ...
    def forward(self, embeds1, embeds2):
        return self.fusion(torch.cat([ embeds1, embeds2 ], dim=1))
...
class ClaimImageVerificationModel(L.LightningModule):
    def __init__(
        self, 
        text_encoder_card,
        image_encoder_card,
        ...
        self.fusion = _FusionModel(self.image_encoder.hidden_size, self.text_encoder.hidden_size)

        self.classifier = _ClassifierModel(self.out_classes, self.fusion.hidden_size)
    ...
    def preprocess(self, inputs):
        return self.text_encoder.preprocess(inputs["claim"]), self.image_encoder.preprocess(inputs["image"])


    def forward(self, text_inputs, image_inputs, training=False):
        ...
        fusion_embeds = self.fusion(text_embeds[:, 0], image_embeds[:, 0])
    ...
    def __metrics(self, logits, labels, log_prefix):
        ...
        self.log(f"{log_prefix}_acc", acc, sync_dist=True)
        self.log(f"{log_prefix}_f1macro", f1, sync_dist=True)


    def training_step(self, batch, batch_idx):
        inputs, labels = batch
        
        text_inputs, image_inputs = self.preprocess(inputs)
        logits, text_sequence_lengths, text_weights, image_weights = self.forward(text_inputs, image_inputs, training=True)

        loss = self.__loss(logits, labels, text_sequence_lengths, text_weights, image_weights, "train")
        self.__metrics(logits, labels, "train")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.__metrics(logits, labels, "val")
